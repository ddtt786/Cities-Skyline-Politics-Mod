...
namespace PoliticsMod
{
    ...
    public class OpinionPollingPanel : UIPanel
    {
        ...
        public static void Toggle()
        {
            ...
            {
                PoliticsUserMod.Log("OpinionPollingPanel.Toggle caught: " + ex.Message);
            }
        }
        ...
        public override void Start()
        {
            ...
            {
                PoliticsUserMod.Log("OpinionPollingPanel.Start caught: " + ex.Message);
            }
        }
        ...
        public override void Update()
        {
            ...
            try
            {
                var history = OpinionPolling.GetHistorySnapshot();
                int hCount = history != null ? history.Count : 0;
                int pCount = Config.Parties != null ? Config.Parties.Length : 0;
                if (pCount != _legendPartyCount || hCount != _legendHistoryLen)
                {
                    RefreshLegendAndSubtitle();
                }
            }
            ...
            {
                PoliticsUserMod.Log("OpinionPollingPanel.Update caught: " + ex.Message);
            }
        }
            ...
            try
            {
                ...
                else
                {
                    _subtitle.text = L10n.T(L10nKeys.Polling_Subtitle,
                        OpinionPolling.SampleSize, hCount);
                }
                _legendPartyCount = Config.Parties != null ? Config.Parties.Length : 0;
                _legendHistoryLen = hCount;
            }
            catch (Exception ex)
            {
                PoliticsUserMod.Log("RefreshLegendAndSubtitle caught: " + ex.Message);
            }
        }
        ...
        private void BuildLegend()
        {
            try
            {
                ...
                for (int i = 0; i < n; i++)
                {
                    if (i >= Config.Parties.Length) break;
                    ...
                    x += 18f + Mathf.Max(40f, (p.ShortName != null ? p.ShortName.Length : 0) * 8f);
                }
            }
            catch (Exception ex)
            {
                PoliticsUserMod.Log("BuildLegend caught: " + ex.Message);
            }
        }
        ...
        private void OnGUI()
        {
            try
            {
                ...
                for (int gi = 0; gi < gridPct.Length; gi++)
                {
                    int g = gridPct[gi];
                ...
                // --- X-axis tick labels ---
                int dayOldest = int.MaxValue;
                int dayNewest = int.MinValue;
                for (int i = 0; i < history.Count; i++)
                {
                    if (history[i].DayIndex < dayOldest) dayOldest = history[i].DayIndex;
                    if (history[i].DayIndex > dayNewest) dayNewest = history[i].DayIndex;
                }
                if (dayOldest == int.MaxValue) dayOldest = 0;
                if (dayNewest < dayOldest) dayNewest = dayOldest;
                ...
                for (int p = 0; p < n; p++)
                {
                    if (p < 0 || p >= parties.Length || parties[p] == null) continue;
                ...
                for (int s = 0; s < history.Count; s++)
                {
                    ...
                    float t = span > 0 ? (sample.DayIndex - dayOldest) / (float)span : 0f;
                    float x = plotX + plotW * t;
                    for (int p = 0; p < lim; p++)
                    {
                        if (p < 0 || p >= parties.Length || p >= arr.Length || parties[p] == null) continue;
                        float y = plotY + plotH * (1f - Mathf.Clamp01(arr[p]));
                        GUI.color = parties[p].Color;
                        GUI.DrawTexture(
            ...
            {
                // Never let a GUI exception escalate to an ExceptionPanel modal
                PoliticsUserMod.Log("OpinionPollingPanel.OnGUI suppressed exception: " + ex.Message);
            }
        }

        /// <summary>
        /// Draw a trend line for one party as a series of rotated thin
        /// rectangles, one per (history[i], history[i+1]) segment.
        /// </summary>
                                   ...
                                   float sx, float sy)
        {
            try
            {
                if (history == null || history.Count < 2 || values == null || values.Length == 0) return;
                var parties = Config.Parties;
                if (parties == null || partyIdx < 0 || partyIdx >= parties.Length || parties[partyIdx] == null) return;
                Color32 baseCol = parties[partyIdx].Color;
                ...
                bool havePrev = false;
                int lim = Math.Min(history.Count, values.Length);
                for (int i = 0; i < lim; i++)
                {
                    if (i < 0 || i >= history.Count || i >= values.Length) break;
                    float t = span > 0 ? (history[i].DayIndex - dayOldest) / (float)span : 0f;
                    float x = plotX + plotW * t;
            ...
            {
                PoliticsUserMod.Log("OpinionPollingPanel.DrawTrendLine caught: " + ex.Message);
            }
        }
                                            ...
                                            float thickness, Color col)
        {
            ...
            {
                PoliticsUserMod.Log("OpinionPollingPanel.DrawLineSegment caught: " + ex.Message);
            }
        }
                                                     ...
                                                     int partyIdx, int window)
        {
            try
            {
                if (history == null || history.Count == 0) return new float[0];
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    for (int k = lo; k <= hi; k++)
                    {
                        if (k >= 0 && k < history.Count)
                        {
                            ...
                            if (arr != null && partyIdx >= 0 && partyIdx < arr.Length)
                            {
                                sum += arr[partyIdx];
                                c++;
                            }
            ...
            {
                PoliticsUserMod.Log("OpinionPollingPanel.ComputeRollingAverage caught: " + ex.Message);
                return new float[0];
            }
        }
    }
